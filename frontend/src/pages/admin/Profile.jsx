...
export default function AdminProfile() {
  ...
  // Render instantly from cached session — no loading state
  ...
  // Silently refresh from MongoDB in background
  useEffect(() => {
    ...
    <AdminLayout>
      ...
      <div className="section-card" style={{ maxWidth: 520 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 24 }}>
          <div style={{ width: 64, height: 64, borderRadius: '50%', background: 'linear-gradient(135deg,#8b5cf6,#ec4899)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.8rem', color: 'white', fontWeight: 800 }}>
            {profile.username?.[0]?.toUpperCase()}
          </div>
          <div>
            <div style={{ fontWeight: 700, fontSize: '1.1rem' }}>{profile.username}</div>
            <span className="badge badge-blue">admin</span>
            <span className="badge badge-green" style={{ marginLeft: 6 }}>{profile.status}</span>
          </div>
        </div>
        ...
        {!editing ? (
          <>
            <div style={{ display: 'grid', gap: 12 }}>
              {[['Username', profile.username], ['Email', profile.email], ['Mobile', profile.mobile]].map(([k, v]) => (
